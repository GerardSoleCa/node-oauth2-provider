...
function authorize(options, req, res, next) {
    ...
    if (req.method == 'GET') {
        ...
        options.storage.getClient(client_id, undefined, function (client) {
            ...
            if (!client) {
                res.statusCode = 400;
                debug.error('client_id is not valid');
                return res.end('client_id is not valid');
            } else if (client.redirect_uri != redirect_uri) {
                res.statusCode = 400;
                debug.error('client_id or redirect_uri are not vali');
                return res.end('client_id or redirect_uri are not valid');
            }
...
function token(options, req, res, next) {
    ...
    options.storage.getClient(client_id, client_secret, function (client) {
        console.log("get client token", client);
        if (!client) {
            res.statusCode = 400;
            debug.error('client_id is not valid');
            return res.end('client_id is not valid');
            ...
            res.statusCode = 400;
            debug.error('client_id is not valid');
            return res.end('Client could not be authenticated');
        }
